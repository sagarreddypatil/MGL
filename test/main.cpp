...
// extern "C" int sizeForFormatType(GLenum format, GLenum type, GLenum
// internalformat);
...
void write_pixel(GLenum format, GLenum type, void *ptr, float r, float g,
                 float b, float a) {
...
void *gen3DTexturePixels(GLenum format, GLenum type, GLuint repeat,
                         GLuint width, GLuint height, GLint depth) {
  GLuint pixel_size;
  size_t buffer_size;
  void *buffer;
  RGBA_Pixel *ptr;
  ...
  err = vm_allocate((vm_map_t)mach_task_self(), (vm_address_t *)&buffer_data,
                    buffer_size, VM_FLAGS_ANYWHERE);
  assert(err == 0);
  assert(buffer_data);
...
void *genTexturePixels(GLenum format, GLenum type, GLuint repeat, GLuint width,
                       GLuint height, GLint depth = 1,
...
void bindAttribute(GLuint index, GLuint target, GLuint vbo, GLint size,
...
GLuint createTexture(GLenum target, GLsizei width, GLsizei height = 1,
                     GLsizei depth = 1, const void *pixels = NULL,
                     GLint level = 0, GLint internalformat = GL_RGBA8,
  ...
  case GL_TEXTURE_3D:
    glTexImage3D(target, level, internalformat, width, height, depth, 0, format,
                 type, pixels);
    break;
...
void bufferSubData(GLenum target, GLuint buffer, GLsizei size,
                   const void *ptr) {
...
int test_draw_arrays(GLFWwindow *window, int width, int height) {
  ...
  const char *vertex_shader = GLSL(
      460, layout(location = 0) in vec3 position;
      void main() { gl_Position = vec4(position, 1.0); });

  const char *fragment_shader = GLSL(
      460, layout(location = 0) out vec4 frag_colour;
  ...
  GLuint shader_program = compileGLSLProgram(
      2, GL_VERTEX_SHADER, vertex_shader, GL_FRAGMENT_SHADER, fragment_shader);
...
int test_draw_arrays_uniform1i(GLFWwindow *window, int width, int height) {
  ...
  const char *vertex_shader = GLSL(
      460, layout(location = 0) in vec3 position; void main() {
        gl_Position = vec4(position.x, position.y, position.z, 1.0);
      });
  const char *fragment_shader = GLSL(
      460, layout(location = 0) out vec4 frag_colour;
      layout(location = 0) uniform int mp;
      void main() { frag_colour = vec4(0, 0, float(mp) / 100.0, 1.0); });
...
int test_draw_arrays_uniform1fv(GLFWwindow *window, int width, int height) {
  ...
  const char *fragment_shader = GLSL(
      ...
      void main() { frag_colour = vec4(mp[0], mp[1], mp[2], 1.0); });
...
int test_draw_arrays_uniform4fv(GLFWwindow *window, int width, int height) {
  ...
  const char *fragment_shader = GLSL(
      ...
      void main() { frag_colour = mp[0] * mp[1]; });
...
int test_draw_arrays_uniformMatrix4fv(GLFWwindow *window, int width,
                                      int height) {
  GLuint vbo = 0, vao = 0;

  const char *vertex_shader = GLSL(
      460 core, layout(location = 0) in vec3 position;
      void main() { gl_Position = vec4(position, 1.0); });

  const char *fragment_shader = GLSL(
      460 core, layout(location = 0) out vec4 frag_colour;
...
int test_draw_elements(GLFWwindow *window, int width, int height) {
  ...
  vbo = bindDataToVBO(GL_ARRAY_BUFFER, sizeof(points), points, GL_STATIC_DRAW);
  elem_vbo = bindDataToVBO(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices,
                           GL_STATIC_DRAW);
  vao = bindVAO();
...
int test_draw_elements_vertex_attribute(GLFWwindow *window, int width,
                                        int height) {
  GLuint vbo = 0, elem_vbo = 0, vao = 0;

  const char *vertex_shader = GLSL(
      460, layout(location = 0) in vec3 position;
      layout(location = 1) in vec3 col; layout(location = 0) out vec3 col_out;
      void main() {
  ...
  const char *fragment_shader = GLSL(
      460, layout(location = 0) in vec3 color_in;
  ...
  bindAttribute(0, GL_ARRAY_BUFFER, vbo, 3, GL_FLOAT, false,
                6 * sizeof(GLfloat), NULL);
  bindAttribute(1, GL_ARRAY_BUFFER, vbo, 3, GL_FLOAT, false,
...
int test_draw_arrays_instanced(GLFWwindow *window, int width, int height) {
  ...
  const char *fragment_shader = GLSL(
      460, layout(location = 0) in vec4 color;
      layout(location = 0) out vec4 frag_colour;
...
int test_draw_arrays_instanced_divisor(GLFWwindow *window, int width,
                                       int height) {
  GLuint vbo[2], vao = 0;
  ...
  vbo[0] =
      bindDataToVBO(GL_ARRAY_BUFFER, sizeof(points), points, GL_STATIC_DRAW);
  vbo[1] =
      bindDataToVBO(GL_ARRAY_BUFFER, sizeof(colors), colors, GL_STATIC_DRAW);
  vao = bindVAO();
...
int test_uniform_buffer(GLFWwindow *window, int width, int height) {
  ...
  const char *fragment_shader = GLSL(
      450 core, layout(location = 0) in vec4 color;
      layout(location = 0) out vec4 frag_colour;
  ...
  shader_program = compileGLSLProgram(2, GL_VERTEX_SHADER, vertex_shader,
...
int test_1D_textures(GLFWwindow *window, int width, int height) {
  ...
  const char *vertex_shader = GLSL(
      450 core, layout(location = 0) in vec3 position;
      layout(location = 1) in float in_texcords;
      layout(location = 0) out float out_texcoords;
      layout(binding = 0) uniform matrices { mat4 rotZ; }; void main() {
        gl_Position = rotZ * vec4(position, 1.0);
        out_texcoords = in_texcords;
      });

  const char *fragment_shader = GLSL(
      450 core, layout(location = 0) in float in_texcords;
      layout(location = 0) out vec4 frag_colour;
  ...
  vbo = bindDataToVBO(GL_ARRAY_BUFFER, sizeof(points), points, GL_STATIC_DRAW);
  tex_vbo = bindDataToVBO(GL_ARRAY_BUFFER, sizeof(texcoords), texcoords,
                          GL_STATIC_DRAW);
  ...
  mat_ubo = bindDataToVBO(GL_UNIFORM_BUFFER, sizeof(mat4), &rotZ[0][0],
                          GL_STATIC_DRAW);
  ...
  // generate 1d texture
  GLuint tex;
  tex = createTexture(GL_TEXTURE_1D, 256, 1, 1,
                      genTexturePixels(GL_RGBA, GL_FLOAT, 0x10, 256, 1));
...
int test_2D_textures(GLFWwindow *window, int width, int height) {
  GLuint vbo = 0, col_vbo = 0, tex_vbo = 0, mat_ubo = 0, scale_ubo = 0,
         col_att_ubo = 0, vao = 0;
  ...
  const char *fragment_shader = GLSL(
      450 core, layout(location = 0) in vec4 in_color;
  ...
  vbo = bindDataToVBO(GL_ARRAY_BUFFER, sizeof(points), points, GL_STATIC_DRAW);
  col_vbo =
      bindDataToVBO(GL_ARRAY_BUFFER, sizeof(color), color, GL_STATIC_DRAW);
  tex_vbo = bindDataToVBO(GL_ARRAY_BUFFER, sizeof(texcoords), texcoords,
                          GL_STATIC_DRAW);
  ...
  mat_ubo = bindDataToVBO(GL_ARRAY_BUFFER, sizeof(texcoords), texcoords,
                          GL_STATIC_DRAW);

  float scale = -1.0;
  scale_ubo =
      bindDataToVBO(GL_ARRAY_BUFFER, sizeof(scale), &scale, GL_STATIC_DRAW);

  float att = 1.0;
  col_att_ubo = bindDataToVBO(GL_ARRAY_BUFFER, sizeof(col_att_ubo),
                              &col_att_ubo, GL_STATIC_DRAW);
...
int test_3D_textures(GLFWwindow *window, int width, int height) {
  ...
  glBufferStorage(GL_ARRAY_BUFFER, 36 * 3 * sizeof(float), points,
                  GL_MAP_WRITE_BIT);
  glBindBuffer(GL_ARRAY_BUFFER, 0);

  glCreateBuffers(1, &tex_vbo);
  glNamedBufferStorage(tex_vbo, 36 * 3 * sizeof(float), texcoords,
                       GL_MAP_WRITE_BIT);
...
#define _A 6
  ...
  glNamedBufferStorage(mat_ubo, sizeof(mat4), &mvp[0][0],
...
#define _3d_size 128
  ...
  glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8, _3d_size, _3d_size, _3d_size, 0,
               GL_RGBA, GL_UNSIGNED_BYTE,
               gen3DTexturePixels(GL_RGBA, GL_UNSIGNED_BYTE, 0x10, _3d_size,
                                  _3d_size, _3d_size));
...
int test_2D_array_textures(GLFWwindow *window, int width, int height) {
  ...
  const char *vertex_shader = GLSL(
      450 core, layout(location = 0) in vec2 in_position;
      layout(location = 1) in vec2 in_texcords;
...
#define _2d_array_depth 8
  ...
  glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_RGBA8, _2d_array_size,
                 _2d_array_size, _2d_array_depth);
  ...
  // test subimage path
  GLuint *pixels;
  pixels = (GLuint *)genTexturePixels(GL_RGBA, GL_UNSIGNED_BYTE, 0x08,
                                      _2d_array_size, _2d_array_size,
  ...
  glNamedBufferStorage(pbo, image_size * _2d_array_depth * sizeof(uint32_t),
                       pixels, GL_MAP_WRITE_BIT);
  ...
  for (int i = 0; i < _2d_array_depth; i++) {
    ...
    glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, _2d_array_size,
                    _2d_array_size, 1, GL_RGBA, GL_UNSIGNED_BYTE,
                    (void *)offset);
  }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glDrawElementsInstanced(GL_TRIANGLE_STRIP, 4, GL_UNSIGNED_SHORT, 0,
                            _2d_array_depth);
...
int test_textures(GLFWwindow *window, int width, int height, int mipmap,
                  int use_gen_mipmap, GLuint anisotropic_level = 0,
...
#define _A 10
  ...
  float points[] = {-_A, 0, -_A, -_A, 0, _A, _A, 0, -_A, _A, 0, _A};

  float texcoords[] = {0, 0, 0, 1, 1, 0, 1, 1};

  vbo = bindDataToVBO(GL_ARRAY_BUFFER, 4 * 3 * sizeof(float), points,
                      GL_STATIC_DRAW);
  tex_vbo = bindDataToVBO(GL_ARRAY_BUFFER, 4 * 2 * sizeof(float), texcoords,
                          GL_STATIC_DRAW);
  ...
  mat_ubo = bindDataToVBO(GL_UNIFORM_BUFFER, sizeof(mat4), &mvp[0][0],
                          GL_STATIC_DRAW);
  ...
  if (mipmap) {
    ...
    if (use_gen_mipmap) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, texsize, texsize, 0, GL_RGBA,
                   GL_UNSIGNED_BYTE,
                   genTexturePixels(GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, 0x8,
                                    texsize, texsize));
      ...
      for (int i = 0; size > 0; i++) {
        glTexImage2D(GL_TEXTURE_2D, i, GL_RGBA8, size, size, 0, GL_RGBA,
                     GL_UNSIGNED_BYTE,
                     genTexturePixels(GL_RGBA, GL_UNSIGNED_INT_8_8_8_8,
                                      0x8 >> i, size, size));
...
GLuint draw_to_framebuffer(GLFWwindow *window, int width, int height) {
  ...
  vbo =
      bindDataToVBO(GL_ARRAY_BUFFER, 8 * sizeof(float), points, GL_STATIC_DRAW);

  const char *vertex_shader = GLSL(
      450 core, layout(location = 0) in vec2 position;
  ...
  const char *fragment_shader = GLSL(
      450 core, layout(location = 0) in vec2 texCoord;
      layout(location = 0) out vec4 frag_colour;
...
int test_framebuffer(GLFWwindow *window, int width, int height) {
  ...
  const char *vertex_texture_shader = GLSL(
      450 core, layout(location = 0) in vec3 position;
      layout(location = 1) in vec3 in_color;
      layout(location = 2) in vec2 in_texcords;
  ...
  const char *fragment_texture_shader = GLSL(
      450 core, layout(location = 0) in vec4 in_color;
      layout(location = 1) in vec2 in_texcords;
  ...
  //
  vbo =
      bindDataToVBO(GL_ARRAY_BUFFER, 9 * sizeof(float), points, GL_STATIC_DRAW);
  col_vbo =
      bindDataToVBO(GL_ARRAY_BUFFER, 9 * sizeof(float), color, GL_STATIC_DRAW);
  tex_vbo = bindDataToVBO(GL_ARRAY_BUFFER, 6 * sizeof(float), texcoords,
                          GL_STATIC_DRAW);
  ...
  mat_ubo = bindDataToVBO(GL_UNIFORM_BUFFER, sizeof(mat4), &rotZ[0][0],
                          GL_STATIC_DRAW);
  scale_ubo =
      bindDataToVBO(GL_UNIFORM_BUFFER, sizeof(float), &scale, GL_STATIC_DRAW);
  col_att_ubo =
      bindDataToVBO(GL_UNIFORM_BUFFER, sizeof(float), &att, GL_STATIC_DRAW);
  ...
  GLuint shader_program;
  shader_program =
      compileGLSLProgram(2, GL_VERTEX_SHADER, vertex_texture_shader,
                         GL_FRAGMENT_SHADER, fragment_texture_shader);
  glUseProgram(shader_program);
...
int test_readpixels(GLFWwindow *window, int width, int height) {
  ...
  GLuint tex;
  tex = createTexture(
      GL_TEXTURE_2D, 256, 256, 0,
      genTexturePixels(GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, 0x10, 256, 256));
  glBindTexture(GL_TEXTURE_2D, tex);
...
const char *compute_shader1 = GLSL(
    450 core, layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;
    ...
    layout(binding = 7) uniform Params { int cell_end_y; };
      ...
      if (gid_z == 0) {
        if (gid_y == 0) {
          ...
          for (int y = -1; y < 2; y++) {
            for (int x = -1; x < 2; x++) {
              n[y * 3 + x] =
                  imageLoad(img_output_0, ivec(gid_x + x, gid_y + y));
              if (n[y * 3 + x].w)
                count++;
            }
          }
...
int test_compute_shader(GLFWwindow *window, int width, int height) {
      ...
      void main() {
        ...
        if (gid_z == 0) {
          for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
              n[y * 3 + x] =
                  imageLoad(img_output_0, ivec2(gid_x + x - 1, gid_y + y - 1));
  ...
  glNamedBufferStorage(sbo, 1024 * sizeof(float), NULL,
  ...
  cell_end_y = height / 2;
  glCreateBuffers(1, &pbo);
  glNamedBufferStorage(pbo, sizeof(GLuint), &cell_end_y,
                       GL_MAP_READ_BIT | GL_MAP_WRITE_BIT |
                           GL_CLIENT_STORAGE_BIT);
  ...
  glNamedBufferStorage(abo, sizeof(GLuint), NULL,
                       GL_MAP_READ_BIT | GL_MAP_WRITE_BIT |
                           GL_CLIENT_STORAGE_BIT);
  ...
  for (int i = 0; i < 2; i++) {
    ...
    glTexSubImage2D(
        GL_TEXTURE_2D, 0, 0, 0, tex_w, tex_h, GL_RGBA, GL_UNSIGNED_BYTE,
...
#define _A 1
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      glBindImageTexture(i, textures[i], 0, GL_FALSE, 0, GL_READ_WRITE,
                         GL_RGBA8);
    }
...
int main_sdl(int argc, const char *argv[]) {
  ...
  SDL_GL_LoadLibrary("/Users/conversy/recherche/istar/code/misc/MGL/build/"
                     "libmgl.dylib"); // this will make SDL_GL_GetProcAdress
                                      // work as expected

  SDL_Window *window = SDL_CreateWindow(
      "MGL Test", 0, 0, 600, 600,
