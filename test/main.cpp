...
int test_draw_arrays_uniform1i(GLFWwindow *window, int width, int height)
{
    ...
    const char *fragment_shader = GLSL(
        460, layout(location = 0) out vec4 frag_colour; layout(location = 0) uniform int mp;
...
int test_draw_arrays_uniform1fv(GLFWwindow *window, int width, int height)
{
    ...
    const char *vertex_shader =
        GLSL(460, layout(location = 0) in vec3 position; void main() { gl_Position = vec4(position, 1.0); });
    const char *fragment_shader = GLSL(
        460, layout(location = 0) out vec4 frag_colour; layout(location = 1) uniform float mp[3];
...
int test_draw_arrays_uniform4fv(GLFWwindow *window, int width, int height)
{
    ...
    const char *vertex_shader =
        GLSL(460, layout(location = 0) in vec3 position; void main() { gl_Position = vec4(position, 1.0); });
    const char *fragment_shader = GLSL(
        460, layout(location = 0) out vec4 frag_colour; layout(location = 1) uniform vec4 mp[2];
...
int test_textures(GLFWwindow *window, int width, int height, int mipmap, int use_gen_mipmap,
                  GLuint anisotropic_level = 0, GLuint min_filter = GL_NEAREST, GLuint mag_filter = GL_NEAREST)
{
    GLuint vbo = 0, tex_vbo = 0, mat_ubo = 0;
...
int test_framebuffer(GLFWwindow *window, int width, int height)
{
    ...
    GLuint shader_program;
    shader_program =
        compileGLSLProgram(2, GL_VERTEX_SHADER, vertex_texture_shader, GL_FRAGMENT_SHADER, fragment_texture_shader);
    glUseProgram(shader_program);
...
const char *compute_shader1 = GLSL(
    ...
    layout(rgba8, binding = 0) uniform image2D img_output_0; layout(rgba8, binding = 1) uniform image2D img_output_1;
    layout(rgba8, binding = 2) uniform image2D img_output_2; layout(rgba8, binding = 3) uniform image2D img_output_3;
    layout(rgba8, binding = 4) uniform image2D img_output_4; layout(rgba8, binding = 5) uniform image2D img_output_5;
...
int main(int argc, const char *argv[])
{
    ...
    SDL_GL_LoadLibrary("/Users/sagar/Documents/MGL/build/libmgl.dylib"); // this will make
    ...
    void *renderer = CppCreateMGLRendererFromContextAndBindToWindow(
        glm_ctx,
        info.info.cocoa.window); // FIXME should do something later with the renderer
    if (!renderer)
    {
